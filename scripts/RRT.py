...
class RRTStar():
    ...
    MAX_ITER = 50                # fixed max number of iterations that the algorithm does
    ...
    def output_best_path(self):
        """Computes the best path from start to goal using the edges in the RRT* tree."""
        done = False
        self.child = self.start
        edge = None  # Initialize edge to avoid UnboundLocalError

        if not self.edges:
            rospy.logwarn("No edges found in RRT* tree. Cannot compute best path.")
            raise ValueError("RRT* tree is empty. No path found.")
        ...
        while not done:
            ...
            for edge in self.edges:
                if edge[0] == self.child:
                    ...
                    self.child = edge[-1]  # Move to the next node
                    flag = True
                    break  # Exit loop after finding the correct edge

            # Ensure edge is assigned before checking edge[-1]
            if edge and edge[-1] == self.goal:
                done = True

            # Debugging: If no valid path is found, print available edges and exit
            if not flag:
                print(f'Current node: ({round(self.child[0], 2)}, {round(self.child[1], 2)})')
                print("Available edges (endpoints):")
                for e in self.edges:
                    print(f'({round(e[-1][0], 2)}, {round(e[-1][1], 2)})')
                raise ValueError("No valid path found. Possible tree disconnect or incomplete path.")

        # Apply Ant Colony Optimization (ACO) if path is found
    ...
    def compute_next_path_node(self, x_current, executed_path):
        ...
        # min_distance =100000
        # dis_x_curr_from_goal = self.compute_distance(x_current, self.goal)
        # for idx, vertex in enumerate(self.best_vertices):
        #     distance = self.compute_distance(x_current,vertex)
        #     dis_from_goal = self.compute_distance(vertex, self.goal)
        #     if dis_from_goal < dis_x_curr_from_goal:
        #         if  distance < min_distance :
        #             min_distance = distance 
        #             next_path_node = vertex
        #             next_idx = idx
    ...
    def old_path_replanning(self, x_current):
        ...
        while x_current != self.goal:
            ...
            next_path_node = self.compute_next_path_node(x_current)
            ...
            if self.find_obstacle(x_current, next_path_node):
                ...
                X_candi = []
                X_near = self.NeighborNodes(x_current, dist=self.DIS)
                for x_near in X_near:
                    if x_near in x_prev: continue
                    #q_traj = self.unicycle.plan_trajectory(x_current, x_near)['points']
                    q_traj, _, smoothness_cost = self.unicycle.get_path_length_and_smoothness(x_current, x_near, return_q_traj= True)
                    if self.map.collision_free(q_traj):
                        # Compute length
                        length = self.compute_cost(x_near, q_traj)
                        ...
                        # smoothness_cost is in radiants
                        #cost_angles = theta + smoothness_cost
            ...
            #print('x_next:',x_next)
            trajectory= self.unicycle.plan_trajectory(x_current, x_next)['points']
            self.replanned_best_path.append(trajectory)
            x_current = x_next
    ...
    def update_tree_structure(self):
        ...
        # rewire their nodes
        self.rewire_orphan_node(Q_children, Q_children, reconnection_range = self.DIS)

    def rewire_orphan_node(self, orphans, Q_children, reconnection_range):
        for orph in orphans:
            Q_near = self.NeighborNodes(orph, reconnection_range)
            if Q_near == []: raise ValueError("No neighbor candidates were found. Consider increasing the ray of neighbors search")
            Q_cands = list(set(Q_near) - set(Q_children)- set(orphans))
            # print('len q near', len(Q_cands))
            ...
            # print('new orphan cost', self.cost[self.vertices.index(orph)])
            
            # Error management
            idx = self.vertices.index(orph)                     
            if self.cost[idx]==10000000: #raise ValueError("No neighbor candidates were found. Consider increasing the ray of neighbors search")
                """Very special case: the orph is very close to the obstacle and the paths generated for connecting him to other
                parents candidates are all in collision"""

                # remove it because it is a leaf node
                idx = self.vertices.index(orph)
                self.vertices.pop(idx)
                self.cost.pop(idx)
